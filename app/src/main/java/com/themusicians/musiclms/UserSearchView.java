  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    final ArrayAdapter<String> myArrayAdapter =
        new ArrayAdapter<String>(
            UserSearchView.this, android.R.layout.simple_list_item_1, myArrayList);

    DatabaseReference getOther =
        FirebaseDatabase.getInstance()
            .getReference()
            .child("node__user")
            .child(currentUser.getUid())
            .child("viewUser");
    getOther.addValueEventListener(
        new ValueEventListener() {
          @Override
          public void onDataChange(@NonNull DataSnapshot snapshot) {
            viewedUser = snapshot.getValue(String.class);
            DatabaseReference other =
                FirebaseDatabase.getInstance().getReference().child("node__user").child(viewedUser);
